...
VVW is a spatial audio exploration game. Players navigate a 2D maze; audio tracks placed in rooms fade in/out based on line-of-sight and distance. The game runs as a WASM web app on Cloudflare Pages. Audio streams from Cloudflare R2. Albums are authored via the CLI (`vvw-deploy create`).
...
- `ProjectManifest` — serialized album: maze, lighting config, physics config, track list, album metadata.

**Key invariant:** The game plugin never touches audio backends directly. It writes `TrackAudioState` components. The platform layer reads them after `SpatialAudioSet` (e.g. `web_audio_sync` pushes to Web Audio API).
...
**Testing:** Unit tests inline (`#[cfg(test)]`). 26 tests across workspace. CI runs `just ci` (format check, clippy, tests, build).
